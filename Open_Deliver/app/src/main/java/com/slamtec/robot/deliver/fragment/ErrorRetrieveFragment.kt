...
import kotlinx.android.synthetic.main.fragment_error_retrieve.*
import kotlinx.android.synthetic.main.fragment_retrieve.*
import kotlinx.coroutines.*


class ErrorRetrieveFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance() = ErrorRetrieveFragment()
    }
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_error_retrieve, container, false)
    }


    override fun initView() {
        super.initView()
        val failedTasks = mRobotViewModel.deviceError?.failed_tasks?.filter { it.type == "TAKEOUT" || it.type == "COLLECT" }
    ...
    private fun refreshView(failedTasks: List<DeviceError.Task>) {
        failedCount = 1
        bv_error_retrieve_retrieve1?.visibility = View.VISIBLE
        bv_error_retrieve_retrieve2?.visibility = View.GONE
        bv_error_retrieve_retrieve1?.setLocation(failedTasks[0].target)
        bv_error_retrieve_retrieve1?.setBoxState(BoxState.OPEN)
        bv_error_retrieve_retrieve1?.setOnClickListener {
            bv_error_retrieve_retrieve1?.isClickable = false
            if (bv_error_retrieve_retrieve1?.mBoxState == BoxState.OPEN) {
                bv_error_retrieve_retrieve1?.setBoxState(BoxState.OPENING)
                operateBox(
                    BoxCmd(
                        ...
                        override fun onSuccess(box: Cargo.Box) {
                            bv_error_retrieve_retrieve1?.setBoxState(BoxState.CLOSED)
                            bv_error_retrieve_retrieve1?.isClickable = true
                        ...
                        override fun onFailed(box: FailedBox?) {
                            ...
                            failedCount--
                            errorCount++
                            bv_error_retrieve_retrieve1?.setBoxState(BoxState.ERROR)
                            checkRetrieveResult()
                        }
                    })
            }
            if (bv_error_retrieve_retrieve1?.mBoxState == BoxState.CLOSED) {
                bv_error_retrieve_retrieve1?.setBoxState(BoxState.CLOSING)
                operateBox(
                    BoxCmd(
                        failedTasks[0].cargos[0].cargo_id,
                        failedTasks[0].cargos[0].boxes[0],
                        ...
                        override fun onSuccess(box: Cargo.Box) {
                            ...
                            bv_error_retrieve_retrieve1?.setBoxState(BoxState.DONE)
                            failedCount--
                            box?.let {
        ...
        if (failedTasks.size > 1) {
            failedCount = 2
            bv_error_retrieve_retrieve2?.visibility = View.VISIBLE
            bv_error_retrieve_retrieve2?.setLocation(failedTasks[1].target)
            bv_error_retrieve_retrieve2?.setBoxState(BoxState.OPEN)
            bv_error_retrieve_retrieve2?.setOnClickListener {
                bv_error_retrieve_retrieve2?.isClickable = false
                if (bv_error_retrieve_retrieve2?.mBoxState == BoxState.OPEN) {
                    operateBox(
                        BoxCmd(
                            ...
                            override fun onSuccess(box: Cargo.Box) {
                                LogMgr.d(">>>retrieve open box ${box?.id} success<<<")
                                bv_error_retrieve_retrieve2?.setBoxState(BoxState.CLOSED)
                                bv_error_retrieve_retrieve2?.isClickable = true
                                checkRetrieveResult()
                            }
                            ...
                            override fun onFailed(box: FailedBox?) {
                                ...
                                failedCount--
                                errorCount++
                                bv_error_retrieve_retrieve2?.setBoxState(BoxState.ERROR)
                                checkRetrieveResult()
                            }
                        })
                }
                if (bv_error_retrieve_retrieve2?.mBoxState == BoxState.CLOSED) {
                    operateBox(
                        BoxCmd(
                            failedTasks[1].cargos[0].cargo_id,
                            failedTasks[1].cargos[0].boxes[0],
                            ...
                            override fun onSuccess(box: Cargo.Box) {
                                LogMgr.d(">>>retrieve close box ${box?.id} success<<<")
                                bv_error_retrieve_retrieve2?.setBoxState(BoxState.DONE)
                                failedCount--
                                box?.let {
                                    if (StockStatus.valueOf(box.stock_status) != StockStatus.EMPTY) {
                                        bv_error_retrieve_retrieve2?.setBoxState(BoxState.OPEN)
